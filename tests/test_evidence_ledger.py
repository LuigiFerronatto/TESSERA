...
def test_evidence_record_is_deterministic(tmp_path):
    text = "---\nid: project/charter\n---\nO propósito do projeto é escalar experimentação.\n"
    ...
    assert first.memory_id == "project/charter"
...
def test_content_edit_creates_new_evidence_version_but_identity_stays(tmp_path):
    ...
    edited = "---\nid: project/charter\n---\nO propósito do projeto é escalar experimentação com agentes.\n"
    ...
    assert before.memory_id == after.memory_id == "project/charter"
...
def test_ledger_deduplicates_and_queries_by_memory(tmp_path):
    ...
    assert ledger.for_memory("project/charter") == [first]
...
def test_engine_integrates_ledger_provenance_evidence_and_derived_snapshot(tmp_path):
    ...
    engine.write_memory_note(
        ...
        content=(
            "O propósito do projeto é escalar experimentação com agentes autônomos.\n\n"
    ...
    engine.write_memory_note(
        mem_id="project/noise", mem_type="factual", episode_id="ep-noise",
        content="Uma nota geral sobre automação de software.",
        tags=["automation"], entities=[],
    )
    engine.build_index(use_cache=False)

    assert len(engine.evidence_ledger.for_memory("project/charter")) == 1
    assert engine.graph.nodes["project/charter"]["evidence_record"]["memory_id"] == "project/charter"
    ...
    assert any(r["memory_id"] == "project/charter" for r in payload["records"])

    results = engine.retrieve_context("qual o propósito do projeto?", top_n=2)
    charter = next(item for item in results if item["id"] == "project/charter")
    assert charter["provenance"]["memory_id"] == "project/charter"
...
def test_cached_index_rebuilds_same_logical_evidence_ledger(tmp_path):
    engine = TesseraEngine(storage_dir=str(tmp_path))
    engine.write_memory_note(
        mem_id="project/charter", mem_type="factual", episode_id="ep",
        content="O propósito do projeto é escalar experimentação.",
        tags=["project"], entities=[],
    )
    engine.build_index(use_cache=False)
    first_id = engine.evidence_ledger.for_memory("project/charter")[0].evidence_id

    reloaded = TesseraEngine(storage_dir=str(tmp_path))
    reloaded.build_index(use_cache=True)
    second_id = reloaded.evidence_ledger.for_memory("project/charter")[0].evidence_id
...
def test_no_relevant_evidence_means_no_query_specific_evidence(tmp_path):
    ...
    engine.write_memory_note(
        mem_id="project/charter", mem_type="factual", episode_id="ep",
        content="O propósito do projeto é escalar experimentação.", tags=["project"], entities=[],
    )
    engine.build_index(use_cache=False)
