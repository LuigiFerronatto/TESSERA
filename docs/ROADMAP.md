...
| [#93](https://github.com/LuigiFerronatto/TESSERA/issues/93) | M0 active Test Card | open | candidate on `test-card/93-storage-config-parity`; no dependencies | [plain-language record](test-cards/93-storage-configuration-parity.md), [PR Evolution Audit](PR_EVOLUTION_93.md), 13-case golden/matrix suite; current-main hidden out-of-store scan reproduced | `SMOKE_ONLY`; deterministic sanity required, LongMemEval skipped | `IN_PROGRESS` |
