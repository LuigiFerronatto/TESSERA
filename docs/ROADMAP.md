...
| [#95](https://github.com/LuigiFerronatto/TESSERA/issues/95) | M0 candidate | open | active branch `test-card/95-remove-legacy-runtime-coupling`; no dependencies | [plain-language record](test-cards/95-remove-legacy-runtime-coupling.md), [PR Evolution Audit](PR_EVOLUTION_95.md), candidate head pending publication | `SMOKE_ONLY`; deterministic sanity metrics must remain fixed | `IN_PROGRESS` |
