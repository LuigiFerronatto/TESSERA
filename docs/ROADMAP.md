...
| [#115](https://github.com/LuigiFerronatto/TESSERA/issues/115) | absent | open | #74, #95 and #112 satisfied; [ADR 0002](adr/0002-repository-layout-and-distribution-boundary.md), [audit](PR_EVOLUTION_115.md) and [plain-language record](test-cards/115-repository-layout-architecture.md) are in the candidate | complete tree/reference/artifact audit; accepted minimal-restructure target; migrations explicitly unimplemented | `SMOKE_ONLY`; deterministic sanity required, LongMemEval skipped | `IN_PROGRESS` |
| [#116](https://github.com/LuigiFerronatto/TESSERA/issues/116) | absent | open | depends on #74, #95 and canonical merged/lifecycle-synchronized #115 | #115 candidate proves benchmark packages are accidentally shipped and tests are partially in sdist; execution must wait for canonical #115 merge | smoke-only when executed | `BLOCKED` |
