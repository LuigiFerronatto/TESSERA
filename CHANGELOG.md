...
- Long or structurally headed Markdown/plain-text sources now retain their
  ...
  Retrieval can use segment matches to rank the parent document and returns
  the parent rather than noisy segment pseudo-memories. Cache reuse now checks
  exact source paths and hashes so moves and timestamp-preserving edits cannot
  leave derived segments stale. Query subgraphs and serialized ranking scores
  are stabilized across Python hash seeds for repeatable evaluation artifacts.
...
- Plain-text `.txt` files are now first-class, body-only knowledge sources in
  ...
  Markdown-only write contract remain unchanged. ([#69](https://github.com/LuigiFerronatto/TESSERA/issues/69))
...
- Root-bounded, read-only project source discovery now returns deterministic
  ...
  indexes, unsafe symlinks, special files and high-confidence credential/key
  artifacts, and exposes the plan through `tessera config doctor --json`.
  ([#154](https://github.com/LuigiFerronatto/TESSERA/issues/154))
...
### Fixed
- Repeating explicit named-global initialization inside a configured project
  now preserves the accepted global store, and unselected symlinks to selected
  files no longer cause false config-doctor or repeat-init failures. Symlinks
  remain excluded from the source corpus.
  ([#118](https://github.com/LuigiFerronatto/TESSERA/issues/118))
...
- Write-gate decisions now separate detection, admission and persistence,
  ...
  sanitization. Python, CLI, MCP and Markdown metadata share the same audited
  result contract. ([#92](https://github.com/LuigiFerronatto/TESSERA/issues/92))
