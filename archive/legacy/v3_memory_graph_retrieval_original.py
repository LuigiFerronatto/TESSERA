    ...
    def audit_and_sanitize(self, content_text: str, tags: List[str]) -> Tuple[str, float, bool]:
        ...
        for pattern in malicious_patterns:
            if re.search(pattern, content_text):
                threat_score += 1.10  # Penalidade alta para tentativas de injeção direta
        ...
        if threat_score > self.toxicity_threshold:
            ...
            sanitized_text = re.sub(r"(?i)ignore as regras anteriores.*$", "[CONTEÚDO REMOVIDO POR INFRAÇÃO DE SEGURANÇA]", sanitized_text)
            sanitized_text = re.sub(r"(?i)ignore.*diretriz.*$", "[CONTEÚDO REMOVIDO POR INFRAÇÃO DE SEGURANÇA]", sanitized_text)
            is_sanitized = True
...
class TesseraEngine:
    ...
    def retrieve_context(self, query_text: str, top_n: int = 3, resolve_conflicts: bool = True) -> List[Dict[str, Any]]:
        ...
        # ELEGANTE MELHORIA (Tessera V3): Expandir o limite de sementes de 5 para 30 para lidar de forma robusta
        # com conjuntos densos de memórias altamente semelhantes em escala. Isso neutraliza os desvios de peso
        # induzidos pela normalização L2 do TF-IDF e abre todo o horizonte temporal para o resolvedor de conflitos.
        seed_nodes = []
        seed_similarities = {}
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as lao_mem_dir:
        print(f"============================================================")
        print(f"🧪 INICIANDO TESTES DO Tessera ENGINE V3 (LAO PERSISTENCE SYSTEM)")
...
Known Pitfalls: Tentar inicializar o cluster antes que as permissões de gravação estejam atribuídas.""",
        ...
        print(saved_content)
        print("-" * 60)
        print("🎉 TESTES EXECUTADOS COM SUCESSO! O Tessera Engine V3 está pronto para produção.")
        print(f"============================================================")
